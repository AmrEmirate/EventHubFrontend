...
import { Calendar, Users, DollarSign, Ticket, Plus, Eye, Edit, MoreHorizontal, CheckCircle, XCircle, Clock, Loader2, Trash2, ExternalLink, Tag } from "lucide-react";
            ...
            <div className="min-h-screen bg-muted/20">
                <div className="container mx-auto px-4 py-8">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                           <Button asChild variant="outline">
                                <Link href="/organizer/create-voucher"><Tag className="mr-2 h-4 w-4" /> Buat Voucher</Link>
                           </Button>
                           <Button asChild>
                    ...
                    <div className="grid gap-4 md:grid-cols-3 mb-8">
                        ...
                        <StatCard title="Total Event" value={`${dashboardData?.stats?.totalEvents ?? 0}`} icon={Calendar} />
                    </div>
                    <Tabs defaultValue="transactions">
                                            ...
                                            <TableBody>
                                                {transactions.length > 0 ? transactions.map((trx) => (
                                                    <TableRow key={trx.id}>
                                                        ...
                                                        <TableCell>
                                                            {trx.status === 'PENDING_CONFIRMATION' && trx.paymentProofUrl && (
                                                                <Button variant="outline" size="sm" asChild>
                                                                    <a href={`${API_BASE_URL}${trx.paymentProofUrl}`} target="_blank" rel="noopener noreferrer">
                        ...
                        <TabsContent value="analytics" className="mt-6">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <Card>
                                    <CardHeader><CardTitle>Tren Pendapatan Harian (30 Hari Terakhir)</CardTitle></CardHeader>
                                    <CardContent>
                                        <ResponsiveContainer width="100%" height={300}>
                                            <LineChart data={dashboardData?.analytics?.revenuePerDay}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" fontSize={12} tickLine={false} axisLine={false} /><YAxis stroke="#888888" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `Rp${Number(value) / 1000}k`} /><Tooltip formatter={(value: number) => formatPrice(value)} /><Legend /><Line type="monotone" dataKey="total" stroke="#8884d8" name="Pendapatan" /></LineChart>
                                        </ResponsiveContainer>
                                    </CardContent>
                                </Card>
                                <Card>
                                    <CardHeader><CardTitle>Top 10 Event Berdasarkan Penjualan Tiket</CardTitle></CardHeader>
                                    <CardContent>
                                        <ResponsiveContainer width="100%" height={300}>
                                            <BarChart layout="vertical" data={dashboardData?.analytics?.ticketsPerEvent} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                                <CartesianGrid strokeDasharray="3 3" />
                                                <XAxis type="number" />
                                                <YAxis type="category" dataKey="eventName" width={150} fontSize={12} />
